# User Profile (Names, Contact Email, Phone) - UX Discovery
...
2. Have a reliable way to reach every platform user (phone + a second email) when something urgent happens with a brand — a listing suspension, an account-health penalty, a failed campaign — and the person needs to be contacted outside the app.
...
Nasam stores only one "full name" and one login email per user, so the platform cannot address the user correctly in both Arabic and English and has no phone or alternate email to reach them when a brand emergency happens outside the app.
...
3. User edits Arabic name, English name, contact email, phone (country code selector defaults to Saudi Arabia +966, or to the user's already-saved country).
...
- **Contact email equals login email:** allow, but it adds nothing; show a soft hint "This is already your login email" and still save.
...
- **Invalid contact email:** standard format error.
- **Network / server failure on save:** keep the dialog open with values intact, show an error toast, allow retry.

### Empty state
First open after release: localized names prefilled by migration where possible, phone empty with the country selector on +966, contact email empty. Optional fields carry helper text explaining *why* we ask (see Content Needs) — this is the difference between "another form" and "they want to reach me if my store has a problem".
...
- **Contact email** — optional input. Helper: "بريد بديل للتواصل معك إن تعذّر الوصول إليك عبر البريد الأساسي" / "An alternate email if we can't reach you on your login email".
- **Live preview line** — small text under the names: "سيظهر اسمك كالتالي: مساء الخير يا محمد" / "You will appear as: Good evening, Mohamed", re-rendered as they type in the current UI language. Cheap, and it makes the "why two names" question answer itself.

### Tertiary (on demand)
- Nothing hidden. Four inputs and a read-only email do not need progressive disclosure. Resist adding role, avatar upload, password change here for now — those are separate tasks.
...
| `contact_email` | varchar(255) | optional, validated format, not unique |
...
**API:**
- `GET /me` — returns the five new fields alongside the existing ones.
- `PATCH /me` — accepts the five fields; server-side validation: script check per name, phone parsed/validated with libphonenumber for the given country, email format. Login email is not accepted here.
...
- **Visibility of system status:** live "You will appear as" preview + immediate sidebar/greeting update after save.
...
- **Cognitive load:** five fields, one screen, no tabs, helper text explains *why* only where a user might hesitate (names, phone).
...
- **Privacy:** phone and contact email are personal data. They should be visible only to the user and to admins with user-management rights, not to other users of the same organization by default. Confirm with the existing permissions model.
...
- بريد إلكتروني للتواصل (اختياري) / Contact email (optional)
...
- Preview: "سيظهر اسمك كالتالي: {greeting}" / "You will appear as: {greeting}"
...
- "صيغة البريد الإلكتروني غير صحيحة" / "Email format is not valid"
...
1. **Contact email vs. login email:** the request says "email" — confirmed here as an *additional* contact email, since the login email is read-only. If you meant something else (e.g. making login email editable), say so.
2. **Who can see phone/contact email?** Proposed: the user themselves + admins with user-management rights. Should org owners see their own staff's numbers?
3. **Should phone become mandatory for Nasam AMs/GCs** after a grace period? Optional for everyone in v1 as requested.
4. **Phone verification (OTP)** — out of scope for v1; flag if you want it now.
5. **Country list:** all countries, or a short list (SA, AE, KW, QA, BH, OM, EG, TR, JO) first with "more" below? Proposed: full list, GCC + Turkey + Egypt pinned at the top.

## Implementation Checklist

- [ ] DB migration: five nullable columns on `users` + backfill script splitting `full_name` by script
